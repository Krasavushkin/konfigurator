...
export const CableDescription = ({data}: CableDescriptionType) => {
    const description = (data: CableConfigType) => {
        ...
        const isolation = data.additionalOptions.fireResistant ? "керамообразующей кремнийорганической резины" :
            data.additionalOptions.polyethylene ? "сшитого полиэтилена" : shealth;
        ...
        return (`Кабель универсальный на номинальное переменное напряжение 660 В с ${wire} жилами ${wireClass}
                 класса по ГОСТ 22483, с номинальным сечением токопроводящих жил ${data.section} мм² из медных ${tinned} проволок${individualScreen}, ${screen},
                 с изоляцией из ${isolation}${coldResistant}, с оболочкой из ${shealth}${coldResistant}, ${armour}`)
    }
    return (
        <div>
            <h2>Описание кабеля</h2>
